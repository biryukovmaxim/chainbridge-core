...
type DepositHandlers map[[common.AddressLength]byte]DepositHandlerFunc
...
type TronDepositHandler struct {
	handlerMatcher  HandlerMatcher
	depositHandlers DepositHandlers
}

// NewTronDepositHandler creates an instance of TronDepositHandler that contains
// handler functions for processing deposit events
func NewTronDepositHandler(handlerMatcher HandlerMatcher) *TronDepositHandler {
	return &TronDepositHandler{
		handlerMatcher:  handlerMatcher,
		depositHandlers: make(DepositHandlers),
	}
}

func (e *TronDepositHandler) HandleDeposit(sourceID, destID uint8, depositNonce uint64, resourceID types.ResourceID, calldata, handlerResponse []byte) (*message.Message, error) {
...
func (e *TronDepositHandler) RegisterDepositHandler(handlerAddress string, handler DepositHandlerFunc) {
	if handlerAddress == "" {
		return
	}
